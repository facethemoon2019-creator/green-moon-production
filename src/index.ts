...
export default {
 async fetch(request:Request, env:Env):Promise<Response> {
  ...
  if(p==="/admin" || p==="/admin/") return env.ASSETS.fetch(new Request(new URL("/admin/index.html",request.url),request));
...
المنتجات: ${JSON.stringify(products.map((x:any)=>({id:x.id,name:x.name,description:x.description,price:x.price})))}`
  ...
  if(p.startsWith("/api/admin/products/") && (method==="PUT" || method==="DELETE")){
    ...
    const b:any=await request.json();
    await env.DB.prepare(`UPDATE products SET name=?,description=?,image_url=?,price=?,old_price=?,wholesale_price=?,cost_price=?,stock=?,max_qty=?,care_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`)
      .bind(b.name,b.description||"",b.imageUrl||"",Number(b.price)||0,Number(b.oldPrice)||0,Number(b.wholesalePrice)||0,Number(b.costPrice)||0,Number(b.stock)||0,Number(b.maxQty)||99,JSON.stringify(b.care||{}),id).run();
    return json({ok:true});
  }
  ...
  if(p==="/api/admin/products" && method==="POST"){
    ...
    await env.DB.prepare(`
      ...
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
    `).bind(
      b.categoryId||null,b.name,slug,b.description||"",b.imageUrl||"",
      Number(b.price)||0,Number(b.oldPrice)||0,Number(b.wholesalePrice)||0,
      Number(b.costPrice)||0,Number(b.stock)||0,Number(b.maxQty)||99,
      JSON.stringify(b.care||{})
    ).run();
    return json({ok:true});
  }
